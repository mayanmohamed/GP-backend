...
//my store
	...
	public List<Product> showProductForSpecificUser(@PathVariable String userId) {
		try {
	...
	public int AddProduct(@PathVariable String userId, @RequestBody Product product) {
try {
	User user = userDAO.findById(userId).get();
...
return 0;
	...
	public int edite(@PathVariable String userId, @RequestBody Product product) {
try {
 ...
 System.out.println(
			 entityManager
				.createQuery("select c from Product c where id =: productId and UserId =:id")
